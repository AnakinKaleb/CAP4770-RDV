...
Original file is located at
    https://colab.research.google.com/gist/AnakinKaleb/b2db4182d5e6baeb6aa2d45b741f26fd/copy-of-pandas.ipynb
...
eCarsData = pd.read_csv('https://raw.githubusercontent.com/zacharski/datamining-guide/main/data/ElectricCarData_Clean.csv',na_values='-')
...
eCarsData["TopSpeed_MPH"] = (eCarsData['TopSpeed_KmH'] / 1.609344)
...
eCarsData['Range'] = (eCarsData['Range_Km'] / 1.609344)
eCarsData['FastCharge_MPH'] = (eCarsData['FastCharge_KmH'] / 1.609344)
eCarsData.head(10)
...
eCarsData['Efficiency_MkwH'] = (eCarsData['Efficiency_WhKm'] / )
...
bevs = eCarsData.filter(['Brand', 'Model', 'PowerTrain', 'AccelSec', 'TopSpeed_MPH', 'Range', 'Efficiency_MkwH'])
...
bevs2
# The electric cars that have better acceleration than the Porsche 911 997 Turbo S are: The Lucid Air, the Porsche Taycan Turbo S, the Tesla Roadster, the Tesla Model S Perfoemance, and the Tesla Model X Performance.
...
# In terms of personal opinions, I think the difference between how much energy our fridges use and how much electric cars use is irrelavent as to whether or not adoption of electric cars is a good idea. 
# I think this comparison is just an attempt at rhetoric and the information is useless, even if it is correct. Cleaner energy sources is more important than how much energy we use; at least I think so.
...
IEAEV = pd.read_csv('https://raw.githubusercontent.com/zacharski/datamining-guide/main/data/IEA-EV-data.csv',na_values='-')
IEAEV.drop(IEAEV[IEAEV['year'] != 2021].index, inplace = True)
IEAEV.drop(IEAEV[IEAEV['region'] == 'World'].index, inplace = True)
IEAEV.drop(IEAEV[IEAEV['region'] == 'Europe'].index, inplace = True)
IEAEV.drop(IEAEV[IEAEV['category'] != 'Historical'].index, inplace = True)
IEAEV.drop(IEAEV[IEAEV['parameter'] != 'EV sales'].index, inplace = True)
IEAEV.drop(IEAEV[IEAEV['powertrain'] != 'BEV'].index, inplace = True)
IEAEV.sort_values(by=['value'], ascending=False)[:5]

IEAEV.sort_values(by=['value'], ascending=True)[:5]
...
# Approximately 14,900,000 Vehicles sold in the USA in 2021 according to google. 14900000 * 2.5 = 37,250,000
# 37,250,000 > 57,000. Yes the U.S. sold significantly more electric Vehicals than the mean for our dataset.